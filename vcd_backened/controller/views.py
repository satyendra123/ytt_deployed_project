...
def check_vehicle_data(request):
    try:
        ...
        if not gate_id or not event:
            return JsonResponse({"status": "error", "message": "Missing gate_id or event"}, status=400)

        gate_status[gate_id] = event
        CarLog.objects.create(gate_id=gate_id,event=event,timestamp=timezone.now())

        print(f"Inserted Data → Gate: {gate_id}, Event: {event}")
        return JsonResponse({"status": "success", "message": "Event data received"})
...
def check_boom(request):
    ...
    print("Received gate_id:", gate_id)
